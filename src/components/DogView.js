...
function DogView(props) {
    ...
    const { breed, image, needReload } = props;
    const [dogImage, setDogImage] = React.useState(image !== undefined ? image : dogPlaceHolder);
    const [toReload, setrealod] = React.useState(needReload);
    ...
    return <div>
        <div>DOG VIEW {toReload} VIEW</div>
        {breedTitle}
        <img src={dogImage} width='200'></img>
